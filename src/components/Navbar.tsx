  ...
  return (
    <nav className="fixed w-full top-0 z-50">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 md:h-20">
          ...
          >
            ...
            <motion.div
              className="absolute inset-0 bg-gradient-to-r from-accent-cyan to-accent-purple rounded-lg opacity-0 blur"
            ...
            <div className="relative w-6 h-6 flex flex-col justify-around">
              <motion.div
                ...
                className="w-5 h-0.5 bg-accent-cyan"
              />
              <motion.div
                animate={isOpen ? { opacity: 0 } : { opacity: 1 }}
                className="w-5 h-0.5 bg-accent-purple"
              />
              <motion.div
                animate={isOpen ? { rotate: -45, y: -10 } : { rotate: 0, y: 0 }}
                className="w-5 h-0.5 bg-accent-blue"
              />
            </div>
          </motion.button>
        </div>
      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-dark-card/90 to-dark-bg/80 backdrop-blur-lg" />
        
        <div className="relative px-4 py-4 space-y-2 border-t border-accent-cyan/10">
          {NAVIGATION.map((item, idx) => (
            <motion.a
              key={item.name}
              href={item.href}
              ...
              whileHover={{ x: 8 }}
              className="group block px-3 py-3 rounded-lg text-gray-300 hover:text-accent-cyan transition-colors relative overflow-hidden"
            >
              {/* Hover background */}
              <motion.div
                className="absolute inset-0 bg-gradient-to-r from-accent-cyan/10 to-accent-purple/10 rounded-lg -z-10"
